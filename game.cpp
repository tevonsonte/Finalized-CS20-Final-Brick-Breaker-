...
void Stack::push(Ball b) { //add ball / life
	balls.push_back(b);
	index++;
}
...
int Stack::peek() { //peek at size of ball vector
	return balls.size();
}

void Stack::wipe() { //clear / reset (balls / lifetotal)
	balls.clear();
	index = 0;
}
...
void Breakout::display(void) {
	...
	case Gameplay:
		...
		// If level 1 beaten move to level 2 immediately
		if (bricks.size() <= 0 && level <= 2) {
			...
			displayVictory(); //spice up console after game ends
			cout << endl;
			addScore(score);
...
void Breakout::addBall(float x = -1, float y = -1) {
	...
	s.push(b1); //add ball to vector stack
}

void Breakout::drawBalls(void) {
	for (vector<Ball>::iterator it = s.balls.begin(); it != s.balls.end(); ) {
...
void Breakout::levelOneBricks(void) {
	...
	for (int i = 0; i < wallRows; ++i) {
		for (int j = 0; j < wallColmns; ++j) {
			// Set stronger bricks to purple (inner square bricks)
...
void Breakout::levelTwoBricks(void) {
	...
	for (int i = 0; i < wallRows; i++) {
		for (int j = 0; j < wallColmns; j++) {
			// Set stronger bricks to purple inside designated area (inner square)
...
void Breakout::drawScore(void) { //display the score (either Times new roman or Helvetica)
	...
	glRasterPos2f(winWidth - 530, 20);
	char buff[300], *pr;
	pr = buff;
	sprintf_s(buff, "KEYS: 'H' - Add More Lives, 'Q' / 'Esc' - Quit Game, 'R' - Reset Game");
	do glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, *pr); 
	while (*(++pr));
